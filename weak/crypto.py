...
def __sameUp(arr_ll_a, arr_ll_b):
    ...
    # Lower Upper
    arr_lu = []
    for C1, v in enumerate(arr_ll_a):
        arr_lu.append({up(v, 1):up(arr_ll_b[C1], 1)})
    # Upper Lower
    arr_ul = []
    for C1, v in enumerate(arr_ll_a):
        arr_ul.append({up(v, 0):up(arr_ll_b[C1], 0)})
    # Upper Upper
    arr_uu = []
    for C1 in enumerate(arr_ll_a):
        arr_uu.append({v.upper():arr_ll_b[C1].upper()})
...
def __randConvertMap():
    ...
    for C1 in range(26 ** 2):
        arr_ll_rnd = arr_ll_c[secrets.randbelow(len(arr_ll_c) - 1)
                              if (len(arr_ll_c)-1) != 0
                              else 0]
...
def __Str2CM(t):
    ...
    ta = sn(t, 2)
    b = [v for i, v in enumerate(ta) if i % 2 == 0] # B is Before
    a = [v for i, v in enumerate(ta) if i % 2 == 1] # A is After
    ll = []
...
def __keyChange(strkey):
    """Change Key2Key"""
    ta = sn(strkey, 2)
    a = [v for i, v in enumerate(ta) if i % 2 == 0] # B is Before
    b = [v for i, v in enumerate(ta) if i % 2 == 1] # A is After
    ll = []
    for i, v in enumerate(a):
        ll.append({b[i]:v})
    lu, ul, uu = __sameUp(b, a)
    return __CM2Str((ll, lu, ul, uu))
